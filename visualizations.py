...
def get_cm_to_plot(samples, thresh_n):
    if isinstance(samples, pd.DataFrame) and samples.empty or not isinstance(samples, pd.DataFrame) and not samples:
        return None
...
def centreau_website_data(combined, labels, site_id, dateStart, dateEnd=None):
...
def plot_centreau(data, metadata, dateStart, output_dir, labels, logo_path, lod=0, langs=None):
    if langs is None:
